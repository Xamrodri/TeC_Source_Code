...
%==========================================================================
% Outputs from TC - Spatial data
%==========================================================================

load("M:\19_ISTA\1_TC\3_Model_Source\3_Output_files\1_Model_outputs\" + ...
    "Region_(Distributed_Ebro)_SubBasin_(Cinca_Mid)_Forcing_(Chelsa)_RunningDate_(181124_1154)_RunningPeriod_(2021_2021)\" + ...
    "\Cell_data_final\Final_reached_step_Cinca_Mid.mat")
...
rb_H = rb_H*((Ccrown_t/sum(Ccrown_t))');
rb_L = rb_L*((Ccrown_t/sum(Ccrown_t))');
rap_H = rap_H*((Ccrown_t/sum(Ccrown_t))');
rap_L = rap_L*((Ccrown_t/sum(Ccrown_t))');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Area = 1;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% VISUAL RESULT
...
if switch_met == 1
    ...
    subplot(3,1,1);
    set(gca,'FontSize',11);
    plot(NN,Ta_S(1:length(NN)),'r','LineWidth', 1.5);
    hold on; grid on;
    title('Air  Temperature ')
    ylabel('[°C]')
    subplot(3,1,2);
    plot(NN,ea_S(1:length(NN)),'m','LineWidth', 1.5);
    hold on; grid on;
