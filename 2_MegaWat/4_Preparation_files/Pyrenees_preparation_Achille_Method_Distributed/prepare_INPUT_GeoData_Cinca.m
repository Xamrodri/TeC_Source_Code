...
targres = 250;
catchment_name = 'Cinca_Mid';
username = 'Max';  %fugger
root = 'M:/19_ISTA/1_TC/3_Model_Source/2_MegaWat/4_Preparation_files/Pyrenees_preparation_Achille_Method_Distributed/'

% Define path to T&C folder

if strcmp(username,'Max')
    path_tc_git = 'M:/19_ISTA/1_TC/3_Model_Source/2_MegaWat/'; % Path to git repository 
    path_tc = path_tc_git; % 
    fnPOI = [root 'POIs/POIs_PYR_29OCT2024.txt'];
elseif strcmp(username,'jouberto')
    path_tc_git = 'C:\Users\jouberto\Desktop\T&C\TC_setups\'; % Path to git repository 
    path_tc = path_tc_git; %   
    fnPOI = 'POIs/Kyzylsu_POIs.txt';
    root ='C:\Users\jouberto\Desktop';
end 

path_sp =  [root 'DEM/']; % Path of folder where spatial data is stored
...
outlet_nm = 'Cinca_Mid';
gauge_nm = ["Cinca_Mid_Camels", "Cinca_Escalona"];%define table of POI for T&C-output

%define DEM filename
 fnDem = strcat(root,'DEM/3_Whole_region/Pyrenees_region.tif');

%define glaciers & debris shapefiles
fnGlacShp = strcat(root,'Glaciers/2_Glacier_locations_shp/Glaciers_europe_32631_simple.shp');
fnDebShp = strcat(root,'Glaciers/5_Debris_covered_glaciers/Debris_covered_glaciers.shp');

%define debris thickness file
fnDebTh = strcat(root,'Glaciers/6_Debris_covered_glaciers_thickness/Debris_Glacier_Thickness_aligned.tif');

%define glacier thickness file
 fnGlaTh = strcat(root,'Glaciers/4_Glacier_Thickness_raster/Glacier_Thickness_geo_aligned.tif'); % Farinotti 2019

%define vegetation map file
fnVeg = strcat(root,'CORINE/4_CORINE_reprojected/U2018_CLC2018_V2020_20u1_EPSG32631.tif');
fnVeg_data_loc = strcat(root,'CORINE/2_Extra_information/Legendkey_CORINE-2018_V2020_aggregationCH.csv');

%define soil map files
fnPSAN = strcat(root,'SoilGrid/7_Alignment_and_clipped/clipped_PY_sand_0-5cm_190m_EPSG_32631_percentage_250_aligned.tif');
fnPCLA = strcat(root,'SoilGrid/7_Alignment_and_clipped/clipped_PY_clay_0-5cm_190m_EPSG_32631_percentage_250_aligned.tif');;
fnPORG = strcat(root,'SoilGrid/7_Alignment_and_clipped/clipped_PY_soc_0-5cm_190m_EPSG_32631_percentage_250_aligned.tif');;

%define snow depth map file
% fnSND = 'SnowDepth\SND_KYZ_2018-09-09_v1.tif';
load(strcat(root,'SnowCover/4_Snow_Depth/Cinca_Init_Snow_Depth_virtual.mat'));
%fnSND = strcat(root,'SnowCover/4_Snow_Depth/Landsat_SnowCoverHybrid_Kyzylsu_2014-08-30.tif');
...
path_func = [path_tc_git '1_Functions/2_Functions'];
addpath(path_func)
%addpath([path_tc_git '\Scripts'])

%path to topotoolbox
addpath(genpath([path_tc_git '1_Functions/4_topotoolbox']))

%path to store output fil
path_out = [root 'Output/' datestr(datetime("today")) '_' num2str(targres) 'm'];
path_out1 = [path_tc '5_Common_inputs'];
...
%Extent in the pyrenees
fnSubCatch = strcat(root,'DEM/1_Catchments/DEM_SubCinca_Mid_clipped.tif');
Box_area = GRIDobj(fnSubCatch);

% Get the number of rows and columns
[nrows, ncols] = size(Box_area.Z);

% Get the cell size
cellsize = Box_area.cellsize;

% Get the reference matrix
refmat = Box_area.refmat;

% Calculate the extent
xllcorner = refmat(3,1);
yllcorner = refmat(3,2);
xurcorner = xllcorner + (ncols * cellsize);
yurcorner = yllcorner - (nrows * cellsize);
xylims=[xllcorner xurcorner yurcorner  yllcorner ];
...
%==========================================================================
poi_utm = readtable(fnPOI); %read table with coordinates
%{
if poi.x > 180
    poi_utm = poi;
    % convert to DEG
...
%==========================================================================
out_idx = find(string(poi_utm.ID2) == 'OUT');

poi_utm(1,2)
%for i=1:numel(gauge_nm) %for some reason the indexing doesn't work without the loop
gauge_idx = find(string(poi_utm.ID2) == 'OUT');
%end
clear('X_v','Y_v','poi');

%% GLACIER MODULE
%==========================================================================
% DERIVE CLEAR GLACIER MASK and IDs
%==========================================================================
glaciers = shaperead(fnGlacShp);

for i = 1:size(glaciers,1)
    RGI_Id_str = glaciers(i).rgi_id;
    RGI_Id(i) = str2num(RGI_Id_str([20:end]));
end
C = num2cell(RGI_Id');
...
%==========================================================================
% Glacier thickness is not read from file (DEC, 2024)
% It is set as 5m from the map of glaciers calculated before.

%Code from Achille (Modified by Max)
%{
...
%Max code below
glThick =  glaciermaskid;
glThick.Z(glThick.Z > 1) = 5;

%Plot glacier thickness
figure()
imagesc(glThick); caxis([0 5])
colorbar
...
figure()
[idx,d] = snap2stream(streams,table2array([poi_utm(out_idx,'chx'),poi_utm(out_idx,'chy')]),true);
title(['Outlet repositioning (d=',num2str(d),'m)'])
axis on
saveas(gcf,[path_fig '/' SIMnm '_outpos_check.png'])
close(gcf)
figure()
[idx_gauge,d_gauge] = snap2stream(streams,table2array([poi_utm(gauge_idx,'chx'),poi_utm(gauge_idx,'chy')]),true);
...
%Open file with codes and modify values
veg_codes = readtable(fnVeg_data_loc)
for i = 1:size(veg_codes,1)
Veg_map.Z(Veg_map.Z == veg_codes.Class(i)) = veg_codes.AggClass(i)
end
%Check
max(Veg_map)
min(Veg_map)
...
%%% 4) Adding PORG in percentage. Initially in g/Kg
PORG_map.Z = PORG_map.Z/10;
...
ix_soil = find((PSAN_map.Z+PCLA_map.Z+PORG_map.Z) > 0);
ix_nosoil = find((PSAN_map.Z+PCLA_map.Z+PORG_map.Z) == 0);
...
X_v = table2array(poi_utm(:,'chx'));
Y_v = table2array(poi_utm(:,'chy'));
[npoi,~] = size(poi_utm);
...
title('Flow accumulation (pixel)')
scatter(x_v(X_POI), y_v(Y_POI),20,'+r','LineWidth',2)
text(x_v(X_POI), y_v(Y_POI),string(poi_utm.name))
colorbar('off')
title('Drainage basin')
%==========================================================================
...
SOIL_TH = SOIL_TH.*MASK; % Soil: relative thickness based on slope (%)
SOIL_TH = SOIL_TH.Z; %converting the object into a matrix
...
hold on
plot(bas_x,bas_y,'-r');
scatter(T.chx, T.chy,20,'+r','LineWidth',2)
colorbar('off')
title('Drainage basin')
