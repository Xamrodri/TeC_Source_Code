################################################################################
# T&C OUTPUT:
#  READ MONTHLY MAPS & WRITE TO RASTER-BRICKS (.GRD)
#
# 
# TODO: -
# 
...
dir.create(new_dir&'/RASTERS/MONTHLY')
...
idx1<-grep(pattern="START <<OUTPUT_MO_ZZZ_SPATIAL_YYY.dat>>",x=rawfile)
idx2<-grep(pattern="END <<OUTPUT_MO_ZZZ_SPATIAL_YYY.dat>>",x=rawfile)
...
vars<-do.call('c',vars)
vars<-vars[endsWith(vars,'_mo')] ##drop non-relevant variables
vars<-str_remove(vars,'_spatial_mo')
#remove B_H and B_L (Carbon Pool Biomass for high and low vegetation carbon pool)
vars<-vars[-which(vars %in% c('B_H','B_L'))]
...
fn_v<-list.files(pattern='OUTPUT_MO_'&simnm&'_SPATIAL_')
if(reinit){
  fn_v<-c(list.files(pattern='OUTPUT_MO_'&simnm1&'_SPATIAL_'),fn_v)
}
...
for(v in 1:length(vars)){
  ...
  # ##testing
  # LS<-list()
  # for(i in 1:length(fn_v)){
  #   print(fn_v[i])
  #   LS[[i]]<-MAT2RAST(fn_v[i],paste0(VAR2,'.spatial.mo'),m_cell,n_cell)
  # }
  
  cl<-makeCluster(2);  registerDoSNOW(cl)
  print(paste0('*** start: ',Sys.time(),' ***********************************'))
  print('*** '&DOMAIN&': Processing monthly << '&VAR&' >> outputs ('&n&' months)  |||  Variable '&v&' of '&length(vars)&' ***')
  LS<-foreach(i = icount(n),.combine=c,.options.snow=opts,.packages=pkgs) %dopar% {
    MAT2RAST(fn_v[i],paste0(VAR2,'.spatial.mo'),m_cell,n_cell)
  }
  ...
  print('***')
  names(br)<-iDate
  writeRaster(br,file=new_dir&'/RASTERS/MONTHLY/'&VAR&'___'&PERIOD_SH&'___br.grd',overwrite=T)
  rm(VAR,VAR2,br)
}
...
###
print('+++END SCRIPT <<READ MONTHLY MAPS>>+++   '&outnm)
