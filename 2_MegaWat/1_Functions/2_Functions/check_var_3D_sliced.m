...
%forc_names = ["t2m" "d2m" "tp" "strd" "ws10" "sp" "RH" ...
%"SAD1" "SAD2" "SAB1" "SAB2" "PARB" "PARD" ]
% 11     12     13     14    15      16  17  

%% FUNCTION
function check_var_3D_sliced(forcing, MASKn)

if ~(numel(fieldnames(forcing)) == 15 ) 
disp("You have to add [Temp Dew_Temp Prep LW WS Pres es ea RH SAD1 SAD2 SAB1 SAB2 PARB PARD N]")
...
Tmin = min(forcing.t2m(MASKn == 1));
Tmax = max(forcing.t2m(MASKn == 1));

TDPmin = min(forcing.d2m(MASKn == 1));
TDPmax = max(forcing.d2m(MASKn == 1));
...
if any(forcing.d2m(MASKn == 1) > forcing.t2m(MASKn == 1))
...
%% Vapor pressure
if any(forcing.sp(MASKn == 1) > 10000)
...
if any(forcing.ea(MASKn == 1) > 10000)
...
%% Wind speed
if any(forcing.ws10(MASKn == 1) > 100)
mistakes{z,1} = "Error: There are wind speeds over 100 m/s. Wind speed must be in m/s";
z =z+1;
end

if any(forcing.ws10(MASKn == 1) < 0)
mistakes{z,1} = "Error: There are negative values. Wind speed must be in m/s";
z =z+1;
end

%% Relative humidity (before entering ther model)
if any(forcing.RH(MASKn == 1) > 1)
mistakes{z,1} = "Error: Relative humidity must be between 0 and 1. Relative humidity is higher than 100%";
z =z+1;
end
