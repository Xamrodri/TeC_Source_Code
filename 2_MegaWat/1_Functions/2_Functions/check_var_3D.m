...
%% FUNCTION
function check_var_3D(forcing)

if ~(numel(fieldnames(forcing)) == 13 ) 
...
Tmin = min(forcing.t2m, [], "all");
Tmax = max(forcing.t2m, [], "all");

TDPmin = min(forcing.d2m, [], "all");
TDPmax = max(forcing.d2m, [], "all");
...
if any(forcing.d2m > forcing.t2m, "all")
