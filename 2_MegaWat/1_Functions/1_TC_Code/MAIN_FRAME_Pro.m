...
r_litter=zeros(NN,cc_max);
%%%
In_H=zeros(NN,cc_max);In_L=zeros(NN,cc_max);
OH=zeros(NN,cc_max);OL=zeros(NN,cc_max);
Psi_s_H=zeros(NN,cc_max); Psi_s_L=zeros(NN,cc_max);
Tdp_H=zeros(NN,cc_max);Tdp_L=zeros(NN,cc_max);
rap_H=zeros(NN,cc_max);rap_L=zeros(NN,cc_max);
Dr_H=zeros(NN,cc_max);Dr_L=zeros(NN,cc_max);
T_H=zeros(NN,cc_max);T_L=zeros(NN,cc_max);EIn_H=zeros(NN,cc_max);EIn_L=zeros(NN,cc_max);
rb_H=zeros(NN,cc_max);rb_L=zeros(NN,cc_max);
An_H=zeros(NN,cc_max);Rdark_H=zeros(NN,cc_max);
An_L=zeros(NN,cc_max);Rdark_L=zeros(NN,cc_max);
Ci_sunH=zeros(NN,cc_max);Ci_sunL=zeros(NN,cc_max);
Ci_shdH=zeros(NN,cc_max);Ci_shdL=zeros(NN,cc_max);
rs_sunH=zeros(NN,cc_max);rs_sunL=zeros(NN,cc_max);
rs_shdH=zeros(NN,cc_max);rs_shdL=zeros(NN,cc_max);
%%%%%
Vx_H=zeros(NN,cc_max);Vl_H=zeros(NN,cc_max);
Vx_L=zeros(NN,cc_max);Vl_L=zeros(NN,cc_max);
Psi_x_H=zeros(NN,cc_max);Psi_l_H=zeros(NN,cc_max);
Psi_x_L=zeros(NN,cc_max);Psi_l_L=zeros(NN,cc_max);
gsr_H=zeros(NN,cc_max);
Jsx_H=zeros(NN,cc_max); Jxl_H=zeros(NN,cc_max);
Kleaf_H=zeros(NN,cc_max);Kx_H=zeros(NN,cc_max);
gsr_L=zeros(NN,cc_max);
Jsx_L=zeros(NN,cc_max);Jxl_L=zeros(NN,cc_max);
Kleaf_L=zeros(NN,cc_max);Kx_L=zeros(NN,cc_max);
fapar_H=zeros(NN,cc_max);fapar_L=zeros(NN,cc_max);
SIF_H=zeros(NN,cc_max);SIF_L=zeros(NN,cc_max);
...
Rg_L=zeros(NNd,cc_max);    % Growth respiration - low vegetation
RA_L=zeros(NNd,cc_max);    % Autotrophic respiration - low vegetation
Rmc_L=zeros(NNd,cc_max);   % Maitenance respiration - low vegetation
ANPP_L=zeros(NNd,cc_max);  % Above ground net primary production - low vegetation
Rms_L=zeros(NNd,cc_max);   % Maitenance respiration sapwood - low vegetation
Rmr_L=zeros(NNd,cc_max);   % Maitenance respiration roots - low vegetation
PHE_S_L=zeros(NNd,cc_max); % Phenology state - low vegetation 
dflo_L=zeros(NNd,cc_max);  % Days from leaf onset - low vegetation
AgeL_L=zeros(NNd,cc_max);  % Leaf Age - low vegetation
e_rel_L=ones(NNd,cc_max);  % Relative efficiency of the photosynthesis apparatus
SAI_L=zeros(NNd,cc_max);   % Steam area index - low vegetation
hc_L=zeros(NNd,cc_max);    % Vegetation height - low vegetation
...
Bfac_dayL=ones(NNd,cc_max); Bfac_weekL=ones(NNd,cc_max); NPPI_L=zeros(NNd,cc_max); TdpI_L=zeros(NNd,cc_max);
%%%
Sr_H=zeros(NNd,cc_max);  Slf_H=zeros(NNd,cc_max);
Sfr_H=zeros(NNd,cc_max); Swm_H=zeros(NNd,cc_max);  Sll_H=zeros(NNd,cc_max);
Sr_L=zeros(NNd,cc_max); Slf_L=zeros(NNd,cc_max);
Sfr_L=zeros(NNd,cc_max); Swm_L=zeros(NNd,cc_max); Sll_L=zeros(NNd,cc_max);
LAIdead_H=zeros(NNd,cc_max); LAIdead_L=zeros(NNd,cc_max);
Rexmy_H=zeros(NNd,cc_max,3); AgeDL_H=zeros(NNd,cc_max); Nreserve_H=zeros(NNd,cc_max);
Preserve_H=zeros(NNd,cc_max); Kreserve_H=zeros(NNd,cc_max);
rNc_H=zeros(NNd,cc_max);rPc_H=zeros(NNd,cc_max);rKc_H=zeros(NNd,cc_max);
Rexmy_L=zeros(NNd,cc_max,3); AgeDL_L=zeros(NNd,cc_max); Nreserve_L=zeros(NNd,cc_max);
Preserve_L=zeros(NNd,cc_max); Kreserve_L=zeros(NNd,cc_max);
rNc_L=zeros(NNd,cc_max);rPc_L=zeros(NNd,cc_max);rKc_L=zeros(NNd,cc_max);
NBLeaf_H =zeros(NNd,cc_max); PARI_H=zeros(NNd,cc_max,3) ; NBLI_H=zeros(NNd,cc_max);
NBLeaf_L =zeros(NNd,cc_max);PARI_L=zeros(NNd,cc_max,3) ; NBLI_L=zeros(NNd,cc_max);
%%%%%
NupI_H=zeros(NNd,cc_max,3);
NupI_L=zeros(NNd,cc_max,3);
NavlI=zeros(NNd,3);
%%%
RB_L=zeros(NNd,cc_max,7);
RB_H=zeros(NNd,cc_max,7);
NuLit_H =zeros(NNd,cc_max,3);
NuLit_L =zeros(NNd,cc_max,3);
%%%%%
BLit=zeros(NNd,cc_max);
%%%%%
Nuptake_H=zeros(NNd,cc_max);
Puptake_H=zeros(NNd,cc_max);
Kuptake_H=zeros(NNd,cc_max);
FNC_H=zeros(NNd,cc_max);
TexC_H=zeros(NNd,cc_max);TexN_H=zeros(NNd,cc_max);TexP_H=zeros(NNd,cc_max);TexK_H=zeros(NNd,cc_max);
TNIT_H=zeros(NNd,cc_max);TPHO_H=zeros(NNd,cc_max);TPOT_H=zeros(NNd,cc_max);
SupN_H=zeros(NNd,cc_max);SupP_H=zeros(NNd,cc_max);SupK_H=zeros(NNd,cc_max);
%%%%
Nuptake_L=zeros(NNd,cc_max);
Puptake_L=zeros(NNd,cc_max);
Kuptake_L=zeros(NNd,cc_max);
FNC_L=zeros(NNd,cc_max);
TexC_L=zeros(NNd,cc_max);TexN_L=zeros(NNd,cc_max);TexP_L=zeros(NNd,cc_max);TexK_L=zeros(NNd,cc_max);
TNIT_L=zeros(NNd,cc_max);TPHO_L=zeros(NNd,cc_max);TPOT_L=zeros(NNd,cc_max);
SupN_L=zeros(NNd,cc_max);SupP_L=zeros(NNd,cc_max);SupK_L=zeros(NNd,cc_max);
%%%%
ISOIL_H=zeros(NNd,cc_max,18);
ISOIL_L=zeros(NNd,cc_max,18);
...
%% These parameters are strange. They need to be defined when Crock, Cbare, Cwat =1
...
for i=2:NN    
    ...
    if (Datam(i,4)==0) 
        ...
        if OPT_SoilBiogeochemistry == 1
            ...
            [P(j,:),LEAK_NH4(j),LEAK_NO3(j),LEAK_P(j),LEAK_K(j),LEAK_DOC(j),LEAK_DON(j),LEAK_DOP(j),...
                ...
                R_bacteria(j),RmycAM(j),RmycEM(j),Prod_B(j),Prod_F(j),BfixN(j),NavlI(j,:),LitFirEmi(j,:)]= ...
                      BIOGEO_UNIT(...
                P(j-1,:),IS,Zbio,sum(Bio_Zs.*rsd),PHs,Tdp_bio,mean(Ta(pdind)),Psi_bio,Se_bio,Se_fc,VSUM,VTSUM,...
        ...
        for cc=1:length(Ccrown)           
            ...
            if (ZR95_L(cc) > 0) || (ZRmax_L(cc) > 0)
                ...
                [LAI_L(j,cc),B_L(j,cc,:),NPP_L(j,cc),ANPP_L(j,cc),Rg_L(j,cc),RA_L(j,cc),Rms_L(j,cc),Rmr_L(j,cc),Rmc_L(j,cc),PHE_S_L(j,cc),...
                    ...
                    rNc_L(j,cc),rPc_L(j,cc),rKc_L(j,cc),ManIL(cc)]= ...
                            VEGGIE_UNIT(...
    ...
    [V(i,:),Vice(i,:),O(i,:),Oice(i,:),ZWT(i),OF(i),OS(i),OH(i,:),OL(i,:),Psi_s_H(i,:),Psi_s_L(i,:),Rd(i),Qi_out(i,:),WTR(i,:),...
        ...
        POT(i,:)]= ...
        HYDROLOGIC_UNIT( ...
        V(i-1,:),           Oice(i-1,:),        aR,             Zs,             EvL_Zs,   ...
        Inf_Zs,             Zinf,               RfH_Zs,         RfL_Zs,         dz, ...
        Dz,                 ms,                 Kbot,           Pr(i),          Ta(i), ...
        Ds(i),              Ws(i),              zatm,           Ts(i-1),        Ts_under(i-1), ...
        IrD(i),             dt,                 dth,            ea(i),          N(i), ...
        Pre(i),             Tstm0,              LAI_H(j,:),     SAI_H(j,:),     LAI_L(j,:), ...
        SAI_L(j,:),         LAIdead_H(j,:),     LAIdead_L(j,:), Rrootl_H(j,:),  Rrootl_L(j,:), ...
        BLit(j,:),          Sllit,              Kct,            Datam(i,:),     DeltaGMT,  ...
        Lon,                Lat,                t_bef,          t_aft,          Ccrown,  ...
        Cbare,              Crock,              Curb,           Cwat,           Soil_Param, ...
        Interc_Param,       SnowIce_Param,      VegH_Param,     VegL_Param,     Zs_deb, ...
        Deb_Par,            ZR95_H,             ZR95_L,         SAB1(i),        SAB2(i),   ...
        SAD1(i),            SAD2(i),            PARB(i),        PARD(i),        SvF,  ...
        SND(i-1),           snow_alb,           Color_Class,    OM_H,           OM_L,    ...
        PFT_opt_H,          PFT_opt_L,          hc_H(j,:),      hc_L(j,:),      d_leaf_H, ...
        d_leaf_L,           Ca(i),              Oa,             Ci_sunH(i-1,:), Ci_shdH(i-1,:), ...
        Ci_sunL(i-1,:),     Ci_shdL(i-1,:),     e_rel_H(j,:),   e_relN_H(j,:),  e_rel_L(j,:), ...
        e_relN_L(j,:),      e_sno(i-1),         In_H(i-1,:),    In_L(i-1,:),    In_Litter(i-1), ...
        In_urb(i-1),        In_rock(i-1),       SWE(i-1),       In_SWE(i-1),    Tdeb(i-1,:),  ...
        Tdp(i-1,:),         Tdpsnow(i-1,:),     Tdamp(i-1),     Tice(i-1),      WAT(i-1),   ...
        ICE(i-1),           IP_wc(i-1),         ICE_D(i-1),     Cicew(i-1),     Vx_H(i-1,:), ...
        Vl_H(i-1,:),        Vx_L(i-1,:),        Vl_L(i-1,:),    Psi_x_H(i-1,:), Psi_l_H(i-1,:), ...
        Psi_x_L(i-1,:),     Psi_l_L(i-1,:),     FROCK(i-1),     Krock,          Ws_under(i-1), ...
        Tdew(i),            t_sls(i-1),         ros(i-1),       SP_wc(i-1),     fpr,    ...
        Pr_sno(pdind),      Urb_Par,            In_max_urb,     In_max_rock,    K_usle,   ...
        tau_sno(i-1),       Ta(pdind),          Slo_top,        Slo_pot,        Asur,    ...
        Ared,               aTop,               EK(i-1),        q_runon(i),     Qi_in(i,:),  ...
        pow_dis,            a_dis,              Salt(i),        SPAR,           SN,   ...
