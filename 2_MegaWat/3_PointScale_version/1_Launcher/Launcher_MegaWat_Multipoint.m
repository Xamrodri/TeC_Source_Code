...
%% Catchment selection
sel_basin = 3; % ID for catchment selection for variable SITE
...
sel_forc = 2;  % selection of forcing
...
forcing_point = ["Monte_Terminillo","Velino"]; %Change in the future to modelling_pixel variable
...
disp('Error: Your points seems to be out of the study area')
end
...
if topo == 1
    ...
    %% Topography
    load(strcat(folder_path,'4_Preparation_files/4_Geoterrain_MultiPoint/4_Results/1_Velino/',SITE,'_ShF.mat')); % ShF matrix created during pre-processing step
    ...
    Par_points_data = Points(Points.Name == string(POI.Name(loc)),:)
...
INIT_COND_v2(num_cell,m_cell,n_cell,...
   ...
   MASKn,GLH,Slo_top,ksv,Ca,SNOWD,SNOWALB,out, II, ij);
load(out);
else 
INIT_COND_v2(num_cell,m_cell,n_cell,...
   cc_max,ms_max,md_max,...
